...
ADV_System.prototype.viewMesAdjust = function(text) {
	...
	// 表示文字数
	var view_text_num = this.VIEW_TEXT_NUM;
...
ADV_System.prototype.loadPicture = function(data) {
	...
	if( pict_data ){
		...
		for(var key in pict_data.list){
			...
			if(!data.ev){
				// BS
				$gameScreen.movePicture(old_pict_id, 1, val._x, val._y, 100, 100, 0, 0, data.t);
			}
...
ADV_System.prototype.macroRun = function(macro) {
	...
	if( macro.indexOf('@bs ') != -1 ){
		...
		// デフォルト値も指定
		argument = this.makeArg(macro,{x:0, y:0, opa:255, t:1000, wt:0, abs:1, width:100, height:100});
...
Window_Message.prototype.needsNewPageEx = function(textState) {
	textState.index++;
	var output = (!this.isEndOfText(textState) && textState.y + textState.height > this.contents.height);
	textState.index--;
	return output;
};
